...
function preload()
{
  platform = loadImage('tennis_platform.png');

  pingpong_ball_touch_paddel= loadSound('ball_touch_paddel.wav');
...
function move(){
  ...
  if (ball.y >= paddle1Y&& ball.y <= paddle1Y + paddle1Height) {
    ball.dx = -ball.dx+0.5;
    playerscore++;
    pingpong_ball_touch_paddel.play();
  }
  else{
    pcscore++;
    reset();
...
//restart function helps restart button to restart the game
function reset(){
  pcscore = 0;
  playerscore = 0;
  loop();
